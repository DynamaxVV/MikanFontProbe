"""Opt-in v6 inference: separate caps/corners, paired local geometry evidence."""
import argparse
import copy
import hashlib
import json
...
from mikan_font_probe.stroke_feature_matcher import (FeatureReferences, compare_features, decide,
...
def augment(sample, templates, baseline, budget=7, references=None):
    ...
    if not rows or sample.get("polarity") == "light" or any(
        ...
        return {**result, "ranking": [], "top3": [], "feature_state": "unsupported_input",
                "feature_pairs": [], "feature_changed": False, "auto_accept": False}
    refs = references or FeatureReferences(templates)
    faces = {f["postscript"]: f for f in refs.fonts}
    by_key = {r["key"]: r for r in rows}
    sources = {}

    def source(i, features=True):
        if i not in sources:
        ...
        if features and "features" not in sources[i]:
            sources[i]["features"] = extract(sources[i]["gray"], sources[i]["pixels"])
        return sources[i]

    pairs = list(combinations([r["key"] for r in rows[:3]], 2))
    ...
    for a, b in pairs:
        ...
        selected_chars = {sample["glyphs"][i]["character"] for i in indices}
        computed, utilities = {}, []
        for i in available:
            glyph = sample["glyphs"][i]
            ...
            pair = pair_features(x, y)
            computed[i] = (x, y, pair)
            if char not in selected_chars and pair["utility"]:
                utilities.append((pair["utility"]*glyph["quality"]["score"], i))
        ...
        for i in selected:
            ...
            evidence.append({"index": i, "character": sample["glyphs"][i]["character"],
                             ...
                             **compare_features(observed, x, y, pair)})
    ...
    minimum = min(r["appearance_distance"] for r in rows)
    close = {r["key"] for r in rows if r["appearance_distance"] <= minimum+.015}
    state = "separated_features_inconclusive"
    first = details[0] if details else None
    if first and first["winner"]:
        if len(indices) < 3 or minimum > .35 or not {first["a"], first["b"]} <= close:
            state = "separated_features_diagnostic_only_guard"
        elif first["winner"] == rows[1]["key"]:
            rows[0], rows[1] = rows[1], rows[0]
            state = "separated_features_top_two_reranked"
        else:
            state = "separated_features_support_leader"
    for i, row in enumerate(rows, 1):
        row["rank"] = i
    return {**result, "ranking": rows, "top3": rows[:3], "feature_state": state,
            "feature_pairs": details, "feature_changed": rows[0]["key"] != baseline["ranking"][0]["key"],
