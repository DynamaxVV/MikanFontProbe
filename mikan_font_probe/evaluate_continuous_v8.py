...
from mikan_font_probe.continuous_evidence_v8 import axis_width, local_evidence
...
from mikan_font_probe.stroke_correspondence_v7 import FeatureReferences, extract, gray_canvas, pair_features
...
def main():
    ...
    for group, name in (("samples", "data/category-expansion-v3"),
        ...
        pools = {s["qid"]: s for s in json.loads((resolve_repo_path(name)/"pools.json").read_text())["samples"]}
    ...
    provenance = {p: hashlib.sha256((resolve_repo_path(p)).read_bytes()).hexdigest() for p in (
