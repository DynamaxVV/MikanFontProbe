...
from mikan_font_probe.process_regions import ROOT, imwrite
...
def main():
    ...
    provenance = {p: hashlib.sha256((resolve_repo_path(p)).read_bytes()).hexdigest() for p in code}
    ...
    refs = FeatureReferences(resolve_repo_path(name)/"templates")
