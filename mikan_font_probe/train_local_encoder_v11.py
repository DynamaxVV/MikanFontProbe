...
def augment(crop: np.ndarray, rng: np.random.Generator,
    ...
    if not deterministic:
        ...
        # Pure white-on-dark lettering is common in manga captions and is a
        # different domain from a white outline.  Keep it as a minority view
        # so polarity is not treated as font identity.
...
def manifest(records: list[dict], labels: dict) -> dict:
    files = {str(path): hashlib.sha256((resolve_repo_path(path)).read_bytes()).hexdigest()
    ...
    return {"schema": "local_encoder_v11", "seed": SEED,
            ...
            "protocol": {"font_files_only_training": True,
                         ...
                         "render_augmentation": "scale, low-resolution rasterization, blur, morphology, white outline, polarity inversion, noise, JPEG compression",
