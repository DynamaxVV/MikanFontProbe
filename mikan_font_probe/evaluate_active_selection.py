...
from mikan_font_probe.informative_selection import run_selection, ranking_for, explain_confidence
...
def evaluate_with_table(separation, table_files):
    ...
    for relative,expected in freeze.items():
        if hashlib.sha256(resolve_repo_path(relative).read_bytes()).hexdigest()!=expected:
    ...
    paths = [resolve_repo_path("informative_selection.py"),resolve_repo_path("evaluate_active_selection.py"),OUTPUT/"pools.json",OUTPUT/"match_scores.npz",*map(Path,table_files)]
