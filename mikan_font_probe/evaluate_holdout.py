...
def prepare():
    ...
    tracked = [CONFIG,resolve_repo_path("match_font_baseline.py"),resolve_repo_path("structure_font_baseline.py"),resolve_repo_path("build_font_templates.py"),resolve_repo_path("evaluate_holdout.py")]
...
def evaluate():
    ...
    for file,expected in freeze["input_files"].items():
        if digest(resolve_repo_path(file)) != expected:
