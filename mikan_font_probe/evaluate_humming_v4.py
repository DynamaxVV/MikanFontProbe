...
def fingerprints():
    files = [resolve_repo_path(name) for name in CODE]+[BASE/"pools.json", BASE/"templates/catalog.json"]
...
def main():
    ...
    paths = [OUT/"pools.json", OUT/"templates/catalog.json", *[resolve_repo_path(name) for name in CODE]]
    ...
    result = evaluate_active_selection.evaluate_with_table(references,[freeze,resolve_repo_path("geometry_font_matcher.py"),resolve_repo_path("glyph_geometry.py")])
