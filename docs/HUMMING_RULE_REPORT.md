...
- 可调用入口：rank_font_guarded.rank_sample(sample, template_directory, budget=7)。命令示例：`.venv/bin/python -m mikan_font_probe.rank_font_guarded --qid 46`。返回Top-3、两种距离、有效分辨率和证据状态，不返回假概率。
- 核心规则：mikan_font_probe/glyph_geometry.py、mikan_font_probe/geometry_font_matcher.py、mikan_font_probe/resolve_font_ambiguity.py。
