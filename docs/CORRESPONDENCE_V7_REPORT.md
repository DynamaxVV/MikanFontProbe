...
2026-09-21。实验入口已实现：[mikan_font_probe/rank_font_correspondence.py](../mikan_font_probe/rank_font_correspondence.py)。结果：q109「こ」此前漏掉的同笔段端点现在能对应；q108「知」右侧细长闭合框也能在源图与三种模板里定位。**这些改进尚未提高整题Top-1；其他字体类别也未观察到可证实的准确率提升。** 默认识别继续使用v4。
...
实现文件：[闭合框特征](../mikan_font_probe/closed_box_features.py)、[结构对应](../mikan_font_probe/stroke_correspondence_v7.py)、[候选入口](../mikan_font_probe/rank_font_correspondence.py)、[回归导出](../mikan_font_probe/evaluate_correspondence_v7.py)、[实时复验](../mikan_font_probe/verify_correspondence_v7.py)。没有增加依赖。
