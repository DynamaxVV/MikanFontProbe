...
完整结果：data/category-expansion-v3/results.json；名单/历史暴露/冻结哈希：protocol.json；字体表：discrimination.json。构建及评估入口mikan_font_probe/expand_font_categories.py，报告入口mikan_font_probe/report_category_expansion.py。首次结果拒绝覆盖，旧测试和字体文件保留。
