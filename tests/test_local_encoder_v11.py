...
from mikan_font_probe.train_local_encoder_v11 import _white_outline, augment, build_records, tight_gray
...
class LocalEncoderV11Tests(unittest.TestCase):
    ...
    def test_outline_augmentation_contains_separate_fill_and_border(self):
        image = np.full((64, 64), 255, np.uint8)
        image[20:44, 20:44] = 0
        outlined = _white_outline(image, np.random.default_rng(7))
        self.assertEqual(outlined.shape, image.shape)
        self.assertLess(int(outlined.min()), 70)
        self.assertGreaterEqual(int(outlined.max()), 220)
