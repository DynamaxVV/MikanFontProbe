...
class RankTests(unittest.TestCase):
    ...
    def test_unused_light_glyph_does_not_erase_ranking(self):
        ...
        with patch("mikan_font_probe.rank_font_correspondence.extract", return_value=[]):
    ...
    def test_guard_reverses_only_near_top_two(self):
        with patch("mikan_font_probe.rank_font_correspondence.extract", return_value=[]), \
                patch("mikan_font_probe.rank_font_correspondence.pair_features", return_value={"utility": 0}), \
                patch("mikan_font_probe.rank_font_correspondence.compare_features", return_value={"vote": -1, "square_anchor": True}):
            result = augment(self.sample, "unused", self.baseline, references=self.refs)
