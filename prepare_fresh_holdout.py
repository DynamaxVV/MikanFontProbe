...
def audit():
    ...
    document = {
        ...
        "prospective_protocol_exceptions": [{"path": name, "sha256": digest((ROOT / name).read_bytes()), "reason": "用户本轮在匹配前冻结的协议及q11资格补充，不是此前使用证据；不豁免任何结果文件"} for name in sorted(PROSPECTIVE_PROTOCOLS) if (ROOT / name).exists()],
...
def reviews(qid, original, glyphs):
    ...
    for index, glyph in enumerate(glyphs):
        crop = read_image(ROOT / glyph["path"])
