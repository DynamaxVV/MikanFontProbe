...
import cache_selection_scores
import evaluate_active_selection
from aligned_discrimination import AlignedSeparation
from process_regions import ROOT
from validate_aligned_holdout import hashes
    ...
    for group in protocol["groups"]:
        for qid in group["selected"]:
            ...
            for glyph in sample["glyphs"]:
                assert hashlib.sha256((ROOT/glyph["path"]).read_bytes()).hexdigest() == glyph["sha256"]
...
def evaluate():
    ...
    result = evaluate_active_selection.evaluate_with_table(lookup, [OUT/"protocol.json", ROOT/"aligned_discrimination.py"])
    lookup.save()
