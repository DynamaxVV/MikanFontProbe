...
- [局部算法](./local_contour_matcher.py)：窗口、轮廓距离、噪声与投票。
- [可运行实验入口](./rank_font_contours.py)：选字、候选比对、前三候选与解释；`--qid 109`。
- [回归导出](./evaluate_contours_v5.py)：冻结、假名表、所有失败证据图；重复实验须指定新的 `--output`，不覆盖完成结果。
- [实时入口验证](./verify_contours_v5.py)：将答案替换为无效标签后重新运行，逐题核对前三、全部局部证据及哈希。
- [单元测试](./test_local_contour_matcher.py)：同图、轮廓差、低分辨率、共享字形、投票和白字拒绝。
